...
get '/' do
  ...
  action = ListStatuses.new StatusJack.new :api
  @statuses = action.do

  # get list of users
  action = ListUsers.new UserJack.new :api
  users = action.do
  @users = {}
  users.each do |user|
    @users[user[:id]] = user
...
post '/:user/create-status' do
  ...
  action = CreateStatus.new StatusJack.new :api
...
post '/sign-up' do
  ...
  action = CreateUser.new UserJack.new :api
...
get '/user/:id' do
  ...
  action = GetUser.new UserJack.new :api
  @user = action.do input
  slim :get_user
end
...
get '/status/:id' do
  input = { :id => params[:id].to_i }
  action = GetStatus.new StatusJack.new :api
  @status = action.do input
...
post '/status/:id/update' do
  ...
  action = UpdateStatus.new StatusJack.new :api
  @status = action.do input
...
post '/status/:id/remove' do
  input = { :id => params[:id].to_i }
  action = RemoveStatus.new StatusJack.new :api
